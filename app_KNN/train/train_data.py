...
class FraudDetectionPipeline:
    ...
    def fit_KNN(self, X_train, y_train):
        """
        Fit a lgbm model using the predefined data preprocessing pipeline.
        ...
        Returns:
        - LGBM Model (LightGBM): The fitted lgbm model.
        """
        self.logfile.debug("Train KNN Model")
        # lgbm_model = RandomForestClassifier(n_estimators=1000, min_samples_split=8, max_depth= 20, criterion='gini', class_weight=None, ccp_alpha= 0.0)
        knn_model = KNeighborsClassifier(n_neighbors=20)
        pipeline = self.create_pipeline()
        # pipeline.fit(X_train, y_train)
        knn_model.fit(pipeline.transform(X_train), y_train)
        return knn_model

...
def train_model():
    ...
    logger.debug(f"ACTION -> Train KNN model ")
    ...
    KNN_model = fraud_data_pipeline.fit_KNN(X_train, y_train)

    result = joblib.dump(KNN_model, MODEL_PATH)
    logger.debug(f"ACTION -> {PIPELINE_NAME} MODEL model saved in {result}")
